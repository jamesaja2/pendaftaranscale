...
import { getUserProfile } from "@/actions/profile"; // Import server action
...
export default function UserDropdown() {
  ...
  React.useEffect(() => {
    // Fetch fresh user data on mount to avoid stale session data
    getUserProfile().then(res => {
         if (res.user) setDbUser(res.user);
    });
  }, []);

  // Use DB user if available, else session
    ...
    <div className="relative">
      ...
      >
        ...
        <span className="block mr-1 font-medium text-theme-sm">
             {userName}
             <span className="block text-xs font-normal text-gray-500">{userEmail}</span>
        </span>
